...
public class StorageTweaksModSystem: ModSystem {
	...
	[
		"ItemSlotSurvival", "ItemSlotBagContent",
	...
	public override void StartClientSide(ICoreClientAPI api)
	{
		...
		api
			.Network.RegisterChannel("storagetweaks")
	...
	public override void StartServerSide(ICoreServerAPI api)
	{
		...
		api
			...
			.SetMessageHandler<QuickStoreNearbyContainersPacket>(
				QuickStoreNearbyContainerSystem.HandleQuickStoreNearbyContainers
			);
	...
	private static void HandleUnloadInventory(IServerPlayer fromPlayer, UnloadInventoryPacket packet)
	{
		...
		{
			Logger().Debug("[StorageTweaks] HandleUnloadInventory: Destination inventory not found");
			return;
		}
	...
	public static void UnloadInventory(IServerPlayer fromPlayer,
		...
		bool stackPerishables = false)
	{
		...
		if (playerInv == null)
		{
			Logger().Debug("[StorageTweaks] HandleUnloadInventory: Player backpack inventory not found");
			return;
		}
		...
		{
			Logger().Debug("[StorageTweaks] HandleUnloadInventory: Player hotbar inventory not found");
			return;
		}
		...
		if (existingCodes.Count == 0)
		{
			Logger()
				.Debug(
					"[StorageTweaks] UnloadInventory: no existing codes in dest ({0}), skipping",
					destInventory.InventoryID
				);
			return;
		}
		...
		Logger()
			.Debug(
				"[StorageTweaks] UnloadInventory: dest={0} class={1} slots={2} existingCodes=[{3}] stackPerishables={4}",
				destInventory.InventoryID,
				destInventory.GetType().Name,
				destInventory.Count,
	...
	private static void ProcessInventorySlots(IInventory sourceInventory,
		IInventory destInventory,
		...
		bool stackPerishables)
	{
		...
		foreach (var slot in sourceInventory)
		{
			...
			Logger()
				.Debug(
					"[StorageTweaks] ProcessSlot: item={0} qty={1} mergePriority={2}",
			...
			while (true)
			{
				var op = new ItemStackMoveOperation(world, EnumMouseButton.Left, 0, mergePriority, slot.StackSize);
				var suitedSlot = destInventory.GetBestSuitedSlot(slot, op, ignoredSlots);
				if (suitedSlot.slot == null || suitedSlot.weight == 0)
				{
					Logger()
						.Debug(
							"[StorageTweaks]   GetBestSuitedSlot returned null/0 (ignoredSlots={0}), trying DirectMerge fallback",
							ignoredSlots.Count
						);
					...
					foreach (var destSlot in destInventory)
					{
						...
						Logger()
							.Debug(
								"[StorageTweaks]     destSlot={0} empty={1} canHold={2} destQty={3} destMax={4}",
								destSlot.GetType().Name,
								destSlot.Empty,
								canHold,
								destSlot.Itemstack?.StackSize ?? 0,
								destSlot.MaxSlotStackSize
							);
						if (!canHold)
						{
						...
						var directOp = new ItemStackMoveOperation(
							world,
						...
						Logger()
							.Debug(
								"[StorageTweaks]     DirectMerge moved={0} slotEmpty={1}",
								directOp.MovedQuantity,
								slot.Empty
							);
	...
	private static void RegisterHotkeys(ICoreClientAPI api)
	{
		...
		api.Input.SetHotKeyHandler(
			...
			_ => {
				...
				PatchUtils.SendPacket(
					api,
					new SortInventoryPacket
					{
		...
		api.Input.SetHotKeyHandler(
			...
			_ => {
				PatchUtils.SendPacket(
					api,
					new QuickStoreNearbyContainersPacket
					{
						StackPerishables = GetClientConfig().StackPerishables
					}
				);
